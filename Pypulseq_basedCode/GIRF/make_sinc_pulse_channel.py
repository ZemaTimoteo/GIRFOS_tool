...
def make_sinc_pulse_channel(channel:str, flip_angle: float, system: Opts = Opts(), duration: float = 0, freq_offset: float = 0,
    ...
    try:
        ...
        g = make_trapezoid(channel=channel, system=system, flat_time=duration, flat_area=area)
        gr = make_trapezoid(channel=channel, system=system, area=-area * (1 - center_pos) - 0.5 * (g.area - area))

        if rf.delay > g.rise_time:
            g.delay = math.ceil((rf.delay - g.rise_time) / system.grad_raster_time) * system.grad_raster_time
