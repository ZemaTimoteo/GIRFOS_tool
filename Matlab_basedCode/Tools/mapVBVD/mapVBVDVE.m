...
% Oscar Pena-Nogales 01/2020 * Added support for acquisitions with a number
%                              of lines larger than 2^16. Typically
%                              diffusion with mulitple slices and multiple
%                              directions.
%
%
% Input:
...
%         NCol_NCha = double( typecast( data_u8(dmaIdx), 'uint32' ) );
...
% -----------------------------------------------------------
if Nmeas > 2^16
    test = uint32(zeros(size(mdh.sLC)));
    test(:,1) = 0:Nmeas-1;
    for i=2:size(test,2)
        test(:,i) = uint32(mdh.sLC(:,i));
    end
    mdh.sLC = test;
end
% -----------------------------------------------------------
