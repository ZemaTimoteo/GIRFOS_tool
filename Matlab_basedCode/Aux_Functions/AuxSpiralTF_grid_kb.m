function [m,nxt,nyt] = grid_kb_teste141118(d,k,w,n,oversample,kbwidth,kbbeta,trimming,apodize,postcompensate)

% function m = grid2w(d,k,w,n,oversample,kbwidth,kbbeta[,trimming[,apodize[,postcompensate]]])
%     d          -- k-space (simulated) data
%     k          -- k-trajectory (created), scaled -0.5 to 0.5
%     w          -- weigths for ADCs in the k-trajectory
...
% min(abs(nx))
% max(abs(nx))
ny_aux = oversample*n*imag(k);
ny     = (oversample*n/2+1) + ny_aux;
% figure(); plot(abs(sort(ny_aux+151)));
% min(ny)
% max(abs(ny))
...
% loop over samples in kernel (selecionar os limites do kernel em x e y)
...
if (postcompensate=='y')
    %disp('Doing post gridding compensation with Auxilliary Array') - eliminating the edges
